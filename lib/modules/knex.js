...
module.exports = function knexInjector(harvester, knex) {
    ...
    return function injectedRunnerQuery() {
      ...
      return _query.apply(this, arguments)
        .then(function harvestDatabaseQuery(queryObject) {
          ...
          var harvestData = {
            duration: ((microtime.now() - startTime) / 1000),
        ...
        .catch(function(err) {
          // harvest the error so we can track issues with database queries
          var harvestData = {
            duration: (microtime.now() - startTime),
            message: err.message,
            code: err.code
          };
          harvester.harvest('db.error', harvestData);
        });
    };
  }
  ...
  function wrapInitialize(initialize) {
    return function injectedInitializer() {
      ...
      harvester._modules[moduleName] = result.VERSION;
